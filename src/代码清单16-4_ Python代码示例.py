...
class AgentEvaluator:
    ...
    def _create_agent(self):
        ...
        # LangChain 1.x：create_agent 取代了 0.x 的 initialize_agent
        # 不再需要 AgentType 枚举，create_agent 内部使用统一的 ReAct 提示
        return create_agent(
            model=self.llm,
            tools=tools,
        )
    ...
    def evaluate_single(self, question: str, criteria: dict) -> dict:
        """评估单个问题"""
        # 1. 获取 Agent 回答
        #    create_agent 返回 LangGraph CompiledStateGraph，入参用 messages，取末条 content
